...
set State:/Network/VPNht/Restore/${SERVICE_ID}
quit
EOF
  ...
  if [[ $SERVICE_SETUP == *"No such key"* ]]; then
    /usr/sbin/scutil <<-EOF > /dev/null
open
remove Setup:/Network/VPNht/Restore/${SERVICE_ID}
quit
EOF
  else
    /usr/sbin/scutil <<-EOF > /dev/null
...
set Setup:/Network/VPNht/Restore/${SERVICE_ID}
quit
EOF
  fi
fi
...
set State:/Network/VPNht/DNS
set State:/Network/VPNht/Connection/${CONN_ID}
quit
EOF
...
/usr/sbin/scutil <<-EOF > /dev/null
open
remove State:/Network/VPNht/Connection/${CONN_ID}
remove State:/Network/VPNht/DNS
quit
EOF
