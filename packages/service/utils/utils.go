...
func RestoreScutilDns() (err error) {
	...
	restoreKey := fmt.Sprintf("/Network/VPNht/Restore/%s", serviceId)
	...
	if strings.Contains(data, "VPNht : true") {
...
func BackupScutilDns() (err error) {
	...
	if strings.Contains(data, "No such key") ||
		strings.Contains(data, "VPNht : true") {
	...
	} else if !strings.Contains(data, "VPNht : true") {
...
func GetScutilConnIds() (ids []string, err error) {
	...
	for _, line := range strings.Split(string(output), "\n") {
		if !strings.Contains(line, "State:/Network/VPNht/Connection/") {
			continue
		}

		spl := strings.Split(line, "State:/Network/VPNht/Connection/")
		if len(spl) == 2 {
...
func ClearScutilKeys() (err error) {
	...
	for _, line := range strings.Split(string(output), "\n") {
		if !strings.Contains(line, "State:/Network/VPNht") {
			continue
		}

		if strings.Contains(line, "State:/Network/VPNht/Restore") {
			continue
		}
...
func GetAuthPath() (pth string) {
	...
	switch runtime.GOOS {
	case "windows":
		pth = filepath.Join("C:\\", "ProgramData", "VPNht")
	...
	case "darwin":
		pth = filepath.Join(string(os.PathSeparator), "Applications",
			"VPN.ht.app", "Contents", "Resources", "auth")
		break
	case "linux":
...
func GetLogPath() (pth string) {
	if constants.Development {
		...
		pth = filepath.Join(pth, "vpnht.log")
	...
	case "darwin":
		pth = filepath.Join(string(os.PathSeparator), "Applications",
			"VPN.ht.app", "Contents", "Resources", "vpnht.log")
		break
	case "linux":
		pth = filepath.Join(string(filepath.Separator),
			"var", "log", "vpnht.log")
...
func GetLogPath2() (pth string) {
	if constants.Development {
		...
		pth = filepath.Join(pth, "vpnht.log.1")
	...
	case "darwin":
		pth = filepath.Join(string(os.PathSeparator), "Applications",
			"VPN.ht.app", "Contents", "Resources", "vpnht.log.1")
		break
	case "linux":
		pth = filepath.Join(string(filepath.Separator),
			"var", "log", "vpnht.log.1")
...
func GetTempDir() (pth string, err error) {
	...
	if runtime.GOOS == "windows" {
		...
		pth = filepath.Join(string(filepath.Separator), "tmp", "vpnht")
...
func GetPidPath() (pth string) {
	if constants.Development {
		...
		pth = filepath.Join(pth, "vpnht.pid")
	...
	case "darwin":
		pth = filepath.Join(string(os.PathSeparator), "Applications",
			"VPN.ht.app", "Contents", "Resources", "vpnht.pid")
		break
	case "linux":
		pth = filepath.Join(string(filepath.Separator),
			"var", "run", "vpnht.pid")
